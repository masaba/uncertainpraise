...
### Read in data
```{r, include = FALSE}
path <- "/Users/masaba/Dropbox/Research/UP_mTurk/" #switch to here? 
...
**Niceness question: Who was trying to be nice?**
```{r}
...
tracing.adult$question = "Tracing"


nicer.adult <- data2.fin %>%
#  mutate(tracing_selective = as.numeric(tracing_selective))%>%
  group_by(condition) %>%
...
tracing.nicer = full_join(tracing.adult, nicer.adult)
tracing.nicer$question = factor(tracing.nicer$question, levels = c("Tracing?", "Nicer?"))
```

### Plots
```{r}

tracing.nicer.plot = ggplot(tracing.nicer, aes(question, mean)) + 
  ...
  geom_linerange(data = tracing.nicer, mapping=aes(x=question, ymin = ci_lower, ymax=ci_upper))+
